...
"""
    Loki module for s2f_adv_wherer
...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "他們在紐約見過面":
        if CHATBOT:
            ...
            resultDICT["fronted"].append("在哪裡"+inputSTR.replace(args[0], "").replace(args[1], ""))
    ...
    if utterance == "張三不喜歡台北":
        if CHATBOT:
            ...
            resultDICT["fronted"].append("哪裡"+inputSTR.replace(args[0], ""))

    if utterance == "張三在新竹看到林瑪莉":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
