...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "每個人都會買衛生紙":
        if CHATBOT:
            ...
            resultDICT["fronted"].append("什麼東西"+inputSTR.replace(args[0], ""))

    if utterance == "王先生在讀過那份文件之後就丟掉了":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
            ...
            resultDICT["fronted"].append("什麼" +args[1]+inputSTR.replace(args[0], "").replace(args[1], ""))

    if utterance == "王小明買了帽子":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
...
if __name__ == "__main__":
    ...
    resultDICT = getResult("王小明買了帽子", "王小明買了帽子", [], {}, {})
    pprint(resultDICT)